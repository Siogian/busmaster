 ...
 */
HRESULT CDIL_CAN_IXXAT_VCI::CAN_GetCurrStatus(STATUSMSG& StatusData)
{
#ifdef _IXXAT_DEBUG
...
HRESULT CDIL_CAN_IXXAT_VCI::CAN_SendMsg(DWORD dwClientID, const STCAN_MSG& sCanTxMsg)
{
    ...
    if ( sCanTxMsg.m_ucChannel > 0 && sCanTxMsg.m_ucChannel <= m_iNumberOfCANChannelsTotal &&
            m_iNumberOfCANChannelsTotal <= defNO_OF_CHANNELS)
    {
 ...
 *  Always S_OK.
 */
HRESULT CDIL_CAN_IXXAT_VCI::CAN_SetAppParams(HWND hWndOwner)
{
